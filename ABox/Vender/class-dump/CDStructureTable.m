...
- (void)finishPhase0;
{
    if (debug) NSLog(@"[%@] %s, changing struct names that start with $", self.identifier, __PRETTY_FUNCTION__);
    ...
    if ([_debugNames count] > 0) {
        NSLog(@"======================================================================");
        NSLog(@"[%@] %s", self.identifier, __PRETTY_FUNCTION__);
...
{
    //NSLog(@"[%@] %s, depth: %u", identifier, __PRETTY_FUNCTION__, depth);
        ...
        if (combined != nil) {
            ...
            if (previousInfo != nil) {
                ...
                if (debugNamedStructures) {
                    NSLog(@"[%@] %s, WARNING: depth %lu name %@ has conflict(?) at lower level", self.identifier, __PRETTY_FUNCTION__, depth, key);
    ...
    for (NSString *key in [anonDict allKeys]) {
        ...
        if (combined != nil) {
            if (_phase2_anonStructureInfo[key] != nil) {
                ...
                NSLog(@"[%@] %s, WARNING: depth %lu type %@ has conflict(?) at lower level", self.identifier, __PRETTY_FUNCTION__, depth, key);
...
- (void)phase2ReplacementOnPhase0;
{
    ...
    if (debug) NSLog(@"[%@] <  %s", self.identifier, __PRETTY_FUNCTION__);
}
...
- (void)runPhase3;
{
    //NSLog(@"[%@]  > %s", identifier, __PRETTY_FUNCTION__);
                   ...
                   usedInMethod:(BOOL)isUsedInMethod
{
    ...
    if ([@"?" isEqualToString:name]) {
        ...
        if (info != nil) {
            if (debugAnonStructures) NSLog(@"%s, anon key %@ has exception from phase 2", __PRETTY_FUNCTION__, structure.typeString);
        ...
        if ([_debugNames containsObject:name]) NSLog(@"[%@] %s, type= %@", self.identifier, __PRETTY_FUNCTION__, structure.typeString);
        //NSLog(@"[%@] %s, name: %@", identifier, __PRETTY_FUNCTION__, name);
        if ([_phase3_exceptionalNames containsObject:name]) {
            if (debugNamedStructures) NSLog(@"%s, name %@ has exception from phase 2", __PRETTY_FUNCTION__, name);
            ...
            if (info == nil) {
                if ([_debugNames containsObject:name]) NSLog(@"[%@] %s, info was nil for %@", self.identifier, __PRETTY_FUNCTION__, name);
                info = [[CDStructureInfo alloc] initWithType:structure];
                [info setReferenceCount:referenceCount];
                if (isUsedInMethod)
                    info.isUsedInMethod = YES;
                ...
                if ([_debugNames containsObject:name]) NSLog(@"[%@] %s, info before: %@", self.identifier, __PRETTY_FUNCTION__, [info shortDescription]);
                ...
                if ([_debugNames containsObject:name]) {
                    NSLog(@"[%@] %s, added ref count: %lu, isUsedInMethod: %u", self.identifier, __PRETTY_FUNCTION__, referenceCount, isUsedInMethod);
                    NSLog(@"[%@] %s, info after: %@", self.identifier, __PRETTY_FUNCTION__, [info shortDescription]);
                }
            }
        }
    }
...
- (BOOL)shouldExpandType:(CDType *)type;
{
    ...
    if ([@"?" isEqualToString:name]) {
        ...
        if (info == nil) {
            ...
            if (info != nil) {
                //NSLog(@"[%@] %s, found phase3 name exception... %@", identifier, __PRETTY_FUNCTION__, [info shortDescription]);
                //return NO;
            }
        }
    }
...
#if 0
    NSLog(@"======================================================================");
    NSLog(@"[%@] %s, named:", identifier, __PRETTY_FUNCTION__);
...
#if 0
    NSLog(@"======================================================================");
    NSLog(@"[%@] %s, anon:", identifier, __PRETTY_FUNCTION__);
...
#if 1
    NSLog(@"======================================================================");
    NSLog(@"[%@] %s, named exceptions:", self.identifier, __PRETTY_FUNCTION__);
...
#if 0
    NSLog(@"======================================================================");
    NSLog(@"[%@] %s, anon exceptions:", identifier, __PRETTY_FUNCTION__);
...
#endif
    NSLog(@"======================================================================");
    NSLog(@"[%@] %s, anon exceptions:", self.identifier, __PRETTY_FUNCTION__);
    for (CDStructureInfo *info in [[_phase3_anonExceptions allValues] sortedArrayUsingSelector:@selector(ascendingCompareByStructureDepth:)]) {
        NSLog(@"%@", [info shortDescription]);
    }
