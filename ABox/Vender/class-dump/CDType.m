...
- (BOOL)canMergeWithType:(CDType *)otherType;
{
    ...
    if (_primitiveType != otherType.primitiveType) {
        ...
        if (debugMerge) NSLog(@"%s, Can't merge because of type... %@ vs %@", __PRETTY_FUNCTION__, self.typeString, otherType.typeString);
        return NO;
    }

    if (_subtype != nil && [_subtype canMergeWithType:otherType.subtype] == NO) {
        if (debugMerge) NSLog(@"%s, Can't merge subtype", __PRETTY_FUNCTION__);
        return NO;
    }

    if (_subtype == nil && otherType.subtype != nil) {
        if (debugMerge) NSLog(@"%s, This subtype is nil, other isn't.", __PRETTY_FUNCTION__);
        return NO;
    }
    ...
    if (count != 0 && otherCount == 0) {
        if (debugMerge) NSLog(@"%s, count != 0 && otherCount is 0", __PRETTY_FUNCTION__);
        return NO;
    }

    if (count != 0 && count != otherCount) {
        if (debugMerge) NSLog(@"%s, count != 0 && count != otherCount", __PRETTY_FUNCTION__);
        return NO;
    }
    ...
    if (count == otherCount) {
        for (NSUInteger index = 0; index < count; index++) { // Oooh
            ...
            if (thisTypeName != nil && otherTypeName != nil && [thisTypeName isEqual:otherTypeName] == NO) {
                if (debugMerge) NSLog(@"%s, typeName mismatch on member %lu", __PRETTY_FUNCTION__, index);
                return NO;
            }

            if (thisVariableName != nil && otherVariableName != nil && [thisVariableName isEqual:otherVariableName] == NO) {
                if (debugMerge) NSLog(@"%s, variableName mismatch on member %lu", __PRETTY_FUNCTION__, index);
                return NO;
            }

            if ([thisMember canMergeWithType:otherMember] == NO) {
                if (debugMerge) NSLog(@"%s, Can't merge member %lu", __PRETTY_FUNCTION__, index);
                return NO;
            }
        }
    }
...
- (void)_recursivelyMergeWithType:(CDType *)otherType;
{
    ...
    if (_primitiveType != otherType.primitiveType) {
        NSLog(@"Warning: Trying to merge different types in %s", __PRETTY_FUNCTION__);
        return;
    }
...
- (void)phase0RecursivelyFixStructureNames:(BOOL)flag;
{
    ...
    if ([_typeName.name hasPrefix:@"$"]) {
        if (flag) NSLog(@"%s, changing type name %@ to ?", __PRETTY_FUNCTION__, _typeName.name);
        _typeName.name = @"?";
    }
...
- (void)phase2MergeWithTypeController:(CDTypeController *)typeController debug:(BOOL)phase2Debug;
{
    ...
    if (phase2Debug && [before isEqualToString:after] == NO) {
        NSLog(@"----------------------------------------");
        NSLog(@"%s, merge changed type", __PRETTY_FUNCTION__);
        NSLog(@"before: %@", before);
        NSLog(@" after: %@", after);
    }
}
...
{
    NSLog(@" > %s %@", __PRETTY_FUNCTION__, [self typeString]);
    for (CDType *member in _members) {
