...
- (int)scanNextToken;
{
    ...
    if ([_scanner isAtEnd]) {
        if (_shouldShowLexing)                       NSLog(@"%s [state=%lu], token = TK_EOS", __PRETTY_FUNCTION__, _state);
        return TK_EOS;
    }
    ...
    if (_state == CDTypeLexerState_TemplateTypes) {
        ...
        if ([_scanner scanString:@"<" intoString:NULL]) {
            if (_shouldShowLexing)                   NSLog(@"%s [state=%lu], token = %d '%c'", __PRETTY_FUNCTION__, _state, '<', '<');
            return '<';
        }
        ...
        if ([_scanner my_scanCharactersFromSet:[NSScanner cdTemplateTypeCharacterSet] intoString:&str]) {
            _lexText = str;
            if (_shouldShowLexing)                   NSLog(@"%s [state=%lu], token = TK_TEMPLATE_TYPE (%@)", __PRETTY_FUNCTION__, _state, _lexText);
        ...
        if ([_scanner scanIdentifierIntoString:&identifier]) {
            _lexText = identifier;
            if (_shouldShowLexing)                   NSLog(@"%s [state=%lu], token = TK_IDENTIFIER (%@)", __PRETTY_FUNCTION__, _state, _lexText);
            _state = CDTypeLexerState_Normal;
        ...
        if ([_scanner scanString:@"\"" intoString:NULL]) {
            ...
            if (_shouldShowLexing)                   NSLog(@"%s [state=%lu], token = TK_QUOTED_STRING (%@)", __PRETTY_FUNCTION__, _state, _lexText);
            return TK_QUOTED_STRING;
        }

        if ([_scanner my_scanCharactersFromSet:[NSCharacterSet decimalDigitCharacterSet] intoString:&str]) {
            _lexText = str;
            if (_shouldShowLexing)                   NSLog(@"%s [state=%lu], token = TK_NUMBER (%@)", __PRETTY_FUNCTION__, _state, _lexText);
            return TK_NUMBER;
        }

        if ([_scanner scanCharacter:&ch]) {
            if (_shouldShowLexing)                   NSLog(@"%s [state=%lu], token = %d '%c'", __PRETTY_FUNCTION__, _state, ch, ch);
            return ch;
        }
    }
