...
{
    //NSLog(@"%s, isProtected? %u, filesize: %lu, fileoff: %lu", __PRETTY_FUNCTION__, [self isProtected], [self filesize], [self fileoff]);
    if (self.isProtected) {
        if (self.filesize <= 3 * PAGE_SIZE) {
            ...
            //NSLog(@"%s, magic= 0x%08x", __PRETTY_FUNCTION__, magic);
            switch (magic) {
