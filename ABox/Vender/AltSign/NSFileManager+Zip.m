...
- (BOOL)writeItemAtURL:(NSURL *)fileURL toZipFile:(zipFile *)zipFile depth:(NSInteger)depth relativeURL:(nullable NSURL *)relativeURL isDirectory:(BOOL)isDirectory error:(NSError **)error
{
    ...
    int compressionLevels = Z_NO_COMPRESSION;
    if ([[NSUserDefaults standardUserDefaults] integerForKey:@"ZipCompressionLevel"] == 1) {
        compressionLevels = Z_BEST_SPEED;
    } else if ([[NSUserDefaults standardUserDefaults] integerForKey:@"ZipCompressionLevel"] == 2) {
        compressionLevels = Z_DEFAULT_COMPRESSION;
    }
    
    if (zipOpenNewFileInZip(*zipFile, filename.fileSystemRepresentation, &fileInfo,
                            NULL, 0, NULL, 0, NULL, Z_DEFLATED, compressionLevels) != ZIP_OK)
    {
